...
export default function CameraFeed({
  ...
  // Post-Capture State Check: True when all poses are captured (e.g. 4/4)
  const isPostCapture =
  ...
  // Update preview image data URL whenever capturedPhotos changes to post-capture state
  useEffect(() => {
    if (isPostCapture) {
  ...
  }, [isPostCapture, generatePhotoStripCanvas]);
  ...
  // Retake Action: Clears capturedPhotos array to [] and returns to live camera feed
  const handleRetake = () => {
    if (activeIntervalRef.current) clearInterval(activeIntervalRef.current);
    if (activeTimeoutRef.current) clearTimeout(activeTimeoutRef.current);
    setCapturedPhotos([]);
  ...
  // DONE Button Handler: Triggers parent component state change (setCurrentStep('preview')) to unmount camera view & mount printer rollout animation
  const handleDone = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-neutral-950 text-white p-3 sm:p-6 flex flex-col items-center justify-between gap-4 max-w-5xl mx-auto selection:bg-orange-500/30">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/30 text-orange-400 text-xs font-semibold">
              {isPostCapture ? `Captured (${capturedPhotos.length}/${selectedLayout.poseCount})` : "Live Feed"}
            </span>
          </div>
        </div>
      ...
      <main className="w-full flex-1 flex flex-col items-center justify-center my-2">
        {/* TOP CONTROL BAR (Only visible during live camera feed) */}
        {!isPostCapture && (
        ...
        {/* CONTROL BAR CONTROL SWITCH: DEFAULT 'START' VS POST-CAPTURE REVIEW BAR */}
        {isPostCapture ? (
          /* POST-CAPTURE CONTROL BAR (Shown when capturedPhotos length === poseCount) */
          <div className="w-full flex items-center justify-center gap-3 sm:gap-4 my-4 flex-wrap overflow-x-auto py-1 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <button
              ...
              className="bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 font-bold rounded-full px-6 py-3 transition-all cursor-pointer text-sm tracking-wide shadow-sm active:scale-95 flex items-center gap-1.5"
            >
              <span>Mirror: {isMirrored ? "On" : "Off"}</span>
            </button>

            {/* 2. Retake (Solid bg-orange-500, white text) -> Clears capturedPhotos array & returns to live feed */}
            <button
              type="button"
            ...
            {/* 3. DONE (White background, border-orange-500 border, text-orange-500 text) -> Triggers parent setCurrentStep('preview') */}
            <button
              type="button"
              onClick={handleDone}
          ...
          /* DEFAULT LIVE CAMERA CONTROL BAR */
